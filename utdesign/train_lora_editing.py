...
from basecode_v4.custom_encoder import FusionModule
...
def save_model_hook_partial(models, weights, output_dir):
    if accelerator.is_main_process and len(weights) > 0:
        ...
        for model in models:
            if config.rank is not None and isinstance(model, type(unwrap_model(transformer))):
...
def load_model_hook_partial(models, input_dir):
    for _ in range(len(models)):
        ...
        if config.rank is not None and isinstance(unwrap_model(model), type(unwrap_model(transformer))):
...
def log_validation(val_dataset, pipeline, config, accelerator):
    ...
    for case_id in config.validation_case_indices:
        ...
        s_refs_w_pil = val_case["s_refs_pil"]
...
def train(global_step, first_epoch, config, accelerator, \
    ...
    for epoch in range(first_epoch, config.num_train_epochs):
        ...
        for step_idx, batch in enumerate(train_dataloader):
            models_to_accumulate = [fusion_module]
            if config.rank is not None:
...
if __name__ == "__main__":
    ...
    if config.rank is not None:
        ...
        fusion_module, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
            fusion_module, optimizer, train_dataloader, lr_scheduler
        )
