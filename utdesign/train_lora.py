...
from custom_dataset.utils.helper import composite_image, pad_and_concat, general_collate_fn
...
def save_model_hook_partial(models, weights, output_dir):
    if accelerator.is_main_process and len(weights) > 0:
        ...
        for model in models:
            ...
            elif not config.get("freeze_fusion", False) and isinstance(unwrap_model(model), type(unwrap_model(fusion_module))):
...
def load_model_hook_partial(models, input_dir):
    for _ in range(len(models)):
        ...
        if isinstance(unwrap_model(model), type(unwrap_model(transformer))):
            ...
            load_model = FusionModule.from_pretrained(input_dir, subfolder="fusion_module", low_cpu_mem_usage=False)
...
def initialize_all_models(config, accelerator):
    ...
    if config.get("freeze_fusion", False):
        fusion_module.to(accelerator.device, dtype=config.weight_dtype)
    ...
    if config.cast_training_params:
        models = [transformer]
        if not config.get("freeze_fusion", False):
            models.append(fusion_module)
        # only upcast trainable parameters (LoRA) into fp32
        cast_training_params(models, dtype=torch.float32)
    
    return vae, transformer, fusion_module
...
def train(global_step, first_epoch, config, accelerator, \
           train_dataloader, optimizer, lr_scheduler, transformer, fusion_module, vae, \
    ...
    for epoch in range(first_epoch, config.num_train_epochs):
        ...
        for step_idx, batch in enumerate(train_dataloader):
            ...
            if not config.get("freeze_fusion", False):
                models_to_accumulate.append(fusion_module)
            with accelerator.accumulate(models_to_accumulate):
                ...
                if random.random() < config.drop_rate:
                    ...
                    s_refs_pil = [[Image.new("RGB", img.size, (255, 255, 255)) for img in item] for item in s_refs_pil]
                ...
                _, ref_style_embeds, ref_pooled_style_embeds, _ = fusion_module(
                    content_images=c_refs_pil,
                    style_images=s_refs_pil,
                )
                max_seq_len = max(style_embeds.shape[1], ref_style_embeds.shape[1])
                c_cond_ids, s_cond_ids = cond_ids
                s_cond_ids = torch.zeros(max_seq_len, 3).to(s_cond_ids)
                ...
                # Dual batch for reference training
                timesteps = torch.cat([timesteps, timesteps], dim=0)
                packed_noisy_model_input = torch.cat([packed_noisy_model_input, packed_noisy_model_input], dim=0)
                content_embeds = torch.cat([content_embeds, content_embeds], dim=0)
                style_embeds, _ = pad_and_concat([style_embeds, ref_style_embeds], dim=1)
                pooled_style_embeds = torch.cat([pooled_style_embeds, ref_pooled_style_embeds], dim=0)
                
                if config.rank is None:
                    packed_noisy_model_input = packed_noisy_model_input.to(dtype=config.weight_dtype)
                    content_embeds = content_embeds.to(dtype=config.weight_dtype)
                    style_embeds = style_embeds.to(dtype=config.weight_dtype)
                    pooled_style_embeds = pooled_style_embeds.to(dtype=config.weight_dtype)
                    timesteps = timesteps.to(dtype=config.weight_dtype)
                    latent_image_ids = latent_image_ids.to(dtype=config.weight_dtype)
                    cond_ids = (cond_ids[0].to(dtype=config.weight_dtype), cond_ids[1].to(dtype=config.weight_dtype))
                ...
                model_pred, ref_model_pred = torch.chunk(model_pred, 2, dim=0)

                # Compute balanced loss
                diff_loss = (model_pred.float() - target.float()) ** 2 + \
                    (ref_model_pred.float() - target.float()) ** 2 * config.lambda_ref
                diff_loss = torch.mean(
                    (weighting.float() * diff_loss).reshape(bsz, n_glyphs, -1),
                    -1,
                )
                diff_loss = diff_loss * valid_mask.to(diff_loss)
                diff_loss = torch.nan_to_num(diff_loss, nan=0.0, posinf=0.0, neginf=0.0)
                diff_loss = diff_loss.clip(0.0, 1.0).mean()

                boosting_loss = (model_pred.float() - ref_model_pred.float().detach()) ** 2
                boosting_loss = torch.mean(
                    (weighting.float() * boosting_loss).reshape(bsz, n_glyphs, -1),
                    -1,
                )
                boosting_loss = boosting_loss * valid_mask.to(boosting_loss)
                boosting_loss = torch.nan_to_num(boosting_loss, nan=0.0, posinf=0.0, neginf=0.0)
                boosting_loss = boosting_loss.clip(0.0, 1.0).mean()

                loss = diff_loss + boosting_loss * config.lambda_boosting
                loss = loss.mean()

                avg_loss = accelerator.reduce(loss, reduction='mean').item()
                avg_diff_loss = accelerator.reduce(diff_loss.mean(), reduction='mean').item()
                avg_boosting_loss = accelerator.reduce(boosting_loss.mean(), reduction='mean').item()
                ...
                if accelerator.sync_gradients:
                    if config.rank is not None:
                        ...
                        params_to_clip = fusion_module.parameters()
            ...
            if accelerator.sync_gradients:
                ...
                logs = {
                    ...
                    "train/boosting_loss": avg_boosting_loss,
                }
...
if __name__ == "__main__":
    ...
    vae, transformer, fusion_module = initialize_all_models(config, accelerator)
