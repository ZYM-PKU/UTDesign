...
if __name__ == "__main__":
    ...
    bg_image = [Image.new('RGB', (224, 224), color='green')]*2
    reg_bboxs = [[0.0, 0.0, 1.0, 1.0] for _ in range(2)]
    prompts = ["a image"]*2
    labels = ["hello"]*2
    ...
    dino_model_name = "facebook/dinov2-large"
    vlm_model_name = "Qwen/Qwen2.5-VL-3B-Instruct"
    clip_cache_dir = "/blob/pretrained/clip"
    vlm_cache_dir = "/blob/pretrained/llamafact"
    ...
    ).to(device=torch.device(f"cuda:7"))
