...
def load_model_hook_partial(models, input_dir):
    for _ in range(len(models)):
        ...
        if isinstance(unwrap_model(model), type(unwrap_model(transformer))):
            load_model = CustomTransformer2DModel.from_pretrained(input_dir, subfolder="transformer")
...
def initialize_all_models(config, accelerator):
    ...
    if config.get("pretrained_ckpt_dir", None):
        transformer = CustomTransformer2DModel.from_pretrained(config.pretrained_ckpt_dir, subfolder="transformer")
        logger.info(f"Loaded transformer from {config.pretrained_ckpt_dir}")
    else:
        transformer_config = config.transformer_config
        transformer: CustomTransformer2DModel = CustomTransformer2DModel.from_config(transformer_config)

    logger.info(f"[INFO] start load fusion module")
    if config.get("pretrained_ckpt_dir", None):
        fusion_module = FusionModule.from_pretrained(config.pretrained_ckpt_dir, subfolder="fusion_module")
        logger.info(f"Loaded fusion_module from {config.pretrained_ckpt_dir}")
    else:
        fusion_module = FusionModule(
            attn_heads=config.fusion_attn_heads,
            dim_head=config.fusion_dim_head,
            clip_model_name=config.clip_model_name, 
            dino_model_name=config.dino_model_name,
            cache_dir=config.cache_dir_clip
        )

    vae.requires_grad_(False)
    transformer.requires_grad_(True)
    fusion_module.requires_grad_(False)
    fusion_module.content_mapper.requires_grad_(True)
    fusion_module.style_mapper.requires_grad_(True)
    fusion_module.pooled_style_mapper.requires_grad_(True)
...
def log_validation(pipeline, config, accelerator):
    ...
    for validation_case in config.validation_cases:
        ...
        style_text = validation_case["style_text"]
        font_base = validation_case["font_base"]
        aug_seed = validation_case.get("aug_seed", None)

        font_path = os.path.join(config.font_root, font_base)
        _, c_refs_w_pil, _ = render_chs(list(text), config.std_font_path, resolution=config.resolution) 
        s_refs_pil, s_refs_w_pil, _ = render_chs(list(style_text), font_path, resolution=config.resolution)
        if aug_seed >= 0:
            s_refs_pil, s_refs_w_pil, _ = aug_chs(s_refs_pil, aug_seed)
        ...
        with torch.autocast(accelerator.device.type, dtype=torch.bfloat16):
            ...
            images = output.images[0]   # list of PIL

        c_refs_concat = np.concatenate([np.array(img) for img in c_refs_w_pil], axis=1)
        s_refs_concat = np.concatenate([np.array(img) for img in s_refs_w_pil], axis=1)
    ...
    for epoch in range(first_epoch, config.num_train_epochs):
        ...
        for step_idx, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                valid_lens = [len(c_text) for c_text in batch["c_text"]]
                ...
                loss = torch.mean(
                    (weighting.float() * loss).reshape(bsz, n_glyphs, -1),
                    -1,
                )
...
if __name__ == "__main__":
    ...
    train_dataset = SynthGlyphDataset(
        datype="train",
        **config.dataset_cfg
    )
